...
performed using *fMRIPrep* 23.1.4
...
BOLD runs were slice-time corrected to 0.968s (0.5 of slice acquisition range
...
Co-registration was configured with twelve degrees of freedomto account for distortions remaining in the BOLD reference.
...
parameters) are estimated before any spatiotemporal filtering using
`mcflirt` [FSL <ver>, @mcflirt].
BOLD runs were slice-time corrected to 1.45s (0.5 of slice acquisition range
0s-2.9s) using `3dTshift` from AFNI  [@afni, RRID:SCR_005927].
The BOLD time-series (including slice-timing correction when applied)
were resampled onto their original, native space by applying
the transforms to correct for head-motion.
These resampled BOLD time-series will be referred to as *preprocessed
