...
def map_compute_stand(file, outdir):
    ...
    # mask
    mask_img = 'GroupMask/group_mean_mask.nii.gz' # standard whole brain mask
    ...
    # zmap
    z_map_dir = os.path.join(outdir, 'zmap_files', motion_file_sub)
    ...
    # beta
    beta_map_dir = os.path.join(outdir, 'beta_files', motion_file_sub)
...
def map_compute_all(args):
    return map_compute_stand(*args)
...
if __name__ == "__main__":
    ...
    import pandas as pd
    events_df = pd.read_csv('Results/CC_ERtiming_stim.csv')
    conditions = events_df['Name'].values
    durations = events_df['Duration'].values
    onsets = events_df['Oneset'].values - 3 * tr
    events = pd.DataFrame(
    ...
    ##############################
    # start - simple
    #glm_dir_simple = 'Results/VolumeFilter_simple'
    #glm_niis_simple = glob(os.path.join(glm_dir_simple, '*/*.nii.gz'))
    #glm_niis_simple = glm_niis_simple[1084:]
    #simple_outdir = ['Results/glm_simple']
    #zip_args_simple = itertools.product(glm_niis_simple, simple_outdir)
    #this_iter_simple = list(zip_args_simple) 
    #run(map_compute_all, this_iter_simple)
    ##############################
    # start - nosimple
    glm_dir_nosimple = 'Results/VolumeFilter_nosimple'
    glm_niis_nosimple = glob(os.path.join(glm_dir_nosimple, '*/*.nii.gz'))
    glm_niis_nosimple = glm_niis_nosimple[967:]
    nosimple_outdir = ['Results/glm_nosimple']
    zip_args_nosimple = itertools.product(glm_niis_nosimple, nosimple_outdir)
    this_iter_nosimple = list(zip_args_nosimple)
    run(map_compute_all, this_iter_nosimple)    
    ##############################
    # start - ica
    glm_dir_ica = 'Results/VolumeFilter_ica'
    glm_niis_ica = glob(os.path.join(glm_dir_ica, '*/*.nii.gz'))
    ica_outdir = ['Results/glm_ica']
    zip_args_ica = itertools.product(glm_niis_ica, ica_outdir)
    this_iter_ica = list(zip_args_ica)
    run(map_compute_all, this_iter_ica)
