...
def map_compute(file):
    ...
    #mask_img = glob(os.path.join('fmriprep', motion_file_sub, 'func','*' + motion_file_pos + '*-MNI152NLin6Asym_res-2_desc-brain_mask.nii.gz'))[0] # error
    # mask
    mask_img = os.path.join('fmriprep', motion_file_sub, 'anat', motion_file_sub+'_space-MNI152NLin6Asym_res-2_desc-brain_mask.nii.gz')  
    fmri_glm = FirstLevelModel(
                    ...
                    drift_order=None,
                    mask_img=mask_img,
                    #smoothing_fwhm=6, # smooth
                    subject_label= motion_file_sub
                )
    fmri_glm = fmri_glm.fit(run_imgs=file, design_matrices=X1)
    # zmap
    z_map_dir = os.path.join('Results/glm/simple/zmap_files', motion_file_sub)
    ...
    # beta
    beta_map_dir = os.path.join('Results/glm/simple/beta_files', motion_file_sub)
...
if __name__ == "__main__":
    ...
    import pandas as pd
    events_df = pd.read_csv('Results/glm/CC_ERtiming_stim.csv')
