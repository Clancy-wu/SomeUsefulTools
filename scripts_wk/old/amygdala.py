...
def get_mask(mask):
    ...
    mask_pos = re.findall(r'(.*)-', mask_full)[0] # position
    mask_size = re.findall(r'-(.*).nii.gz', mask_full)[0] # sieze
...
if __name__ == '__main__':
    #### beta
    masks = glob(os.path.join('mask/AmygdalaMask', 'full*.nii.gz' ))
    beta_files = glob(os.path.join('Results/glm_gm/simple/beta_files', '*','beta_*' ))
    ...
    df_simple.to_csv(os.path.join('Results','glm_gm', 'simple', 'gm_Beta_Amygdala_full_Results.csv'), header=True, index=None)

    #### zmap
    masks = glob(os.path.join('mask/AmygdalaMask', 'full*.nii.gz' ))
    zmap_files = glob(os.path.join('Results/glm_gm/simple/zmap_files', '*','zmap_*' ))
    ...
    df_simple.to_csv(os.path.join('Results','glm_gm', 'simple', 'gm_Zmap_Amygdala_full_Results.csv'), header=True, index=None)
